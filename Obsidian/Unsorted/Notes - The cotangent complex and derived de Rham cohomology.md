...
- Reference: [https://www.msri.org/workshops/862/schedules/25958](https://www.msri.org/workshops/862/schedules/25958).

- Specific Video: <https://www.youtube.com/watch?v=zRPa-VAvl6Q>

## Motivation

Basic affine objects in AG: commutative rings, replace with [simplicial%20commutative%20rings](simplicial%20commutative%20rings) which we'll use as a base diagram.
Later: [geometric%20derived%20stacks](geometric%20derived%20stacks).
Here is an evolution of objects. and how we can think about them:

:::{.example title="Algebraic Spaces"}
[Algebraic%20spaces](Algebraic%20spaces), e.g. $\PP^n$.
Think of these as [étale%20sheaves) of sets (think functor of points](étale%20sheaves)%20of%20sets%20(think%20functor%20of%20points), identified as discrete spaces:
\[  
...
For $X$ a smooth and proper $k\dash$scheme, the [Picard%20stack](Picard%20stack) 
\[
\underline{\Pic}_{X/k}
\]

is an [Deligne-Mumford%20stacks](Deligne-Mumford%20stacks).
...
> Related links: [groupoid.md](groupoid.md).
...
$K(\GG_m, n)$ is a [higher%20stack](higher%20stack), thought of as a sheaf taking values in $n\dash$truncated spaces $\mathcal{S}_{\leq n}$, i.e. a space where when basing at any point, there are no homotopy groups above degree $n$:
\[  
...
:::{.remark}
Note that these are all built from [affine%20scheme.md](affine%20scheme.md) with a few acceptable moves.
:::
...
Mapping into this gives the [Picard%20groupoid](Picard%20groupoid) of a scheme.
These are higher [geometric%20stacks](geometric%20stacks) that still have some "smoothness" properties.
:::
...
Note that the category of [Enriched%20category.md](Enriched%20category.md) in topological spaces.
...
:::{.remark}
Why [derived%20algebraic%20geometry.md](derived%20algebraic%20geometry.md) things?
...
This is a [derived%20scheme](derived%20scheme).
...
For our purposes, they'll be [simplicial%20commutative%20rings](simplicial%20commutative%20rings).
Consider the [derived%20category.md) $\mathcal{C} \da D(\ZZ](derived%20category.md)%20$/mathcal{C}%20/da%20D(/ZZ)_{\geq 0}$ and its connective[^def_connective]
objects, which are chain complexes $C_\wait$ where $H_{< 0}(C_\wait) = 0$.
There is a [derived%20tensor%20product](derived%20tensor%20product) 
$\tensor^L$
which makes $\mathcal{C}$ into a [Symmetric%20monoidal%20category.md](Symmetric%20monoidal%20category.md).
...
- Over $\QQ$, $\QQ\dash$commutative [differential%20graded%20algebras.md](differential%20graded%20algebras.md).
...
$\ssets \homotopic \Top$: this is not an equivalence of categories, but rather they have *equivalent homotopy theories*[Weak%20equivalence.md](Weak%20equivalence.md) [^def_weak_equivalence] on each side.
...
If you make a functor $\Delta^n \mapstofrom \Delta^n_{\text{Top}}$, then by [representable](representable) objects.
...
Considering $\Delta^\wait_{\Top}$, this is a [cosimplicial%20object](cosimplicial%20object) in $\Top$.
...
As in singular homology, we can get a [simplicial%20abelian%20group) by taking the free abelian group $\ZZ[\Sing(X](simplicial%20abelian%20group)%20by%20taking%20the%20free%20abelian%20group%20$/ZZ[/Sing(X)]$.
...
There is a notion of weak equivalence on the RHS, [quasiisomorphism.md](quasiisomorphism.md), and asking if the literal underlying spaces on the LHS are weakly equivalent as spaces.
A specific way of doing this is the [Dold-Kan%20correspondence.md](Dold-Kan%20correspondence.md):
...
This was studied by Quillen, and an impetus for [model%20categories](model%20categories).
Models give a notion of weak equivalence and a "right way" of computing: for the usual [derived%20category.md](derived%20category.md) of a ring, this is taking free/projective/injective resolutions.
...
there is an [adjunction.md](adjunction.md) from sets to commutative $k\dash$algebras
...
Any time such an adjunction exists, given an $R\in \mathrm{CAlg}_k$ we can construct a [simplicial%20resolution.md](simplicial%20resolution.md) $S^\wait$ and a map $S^\wait \to R$.
This resolution has the following structure:
\[  
S^0 = k[R], \,\, \text{the free commutative algebra on }R
.\]
Using the unit and counit maps of the adjunction, one obtains a canonical [simplicial%20object.md](simplicial%20object.md),
\begin{tikzcd}
  k[R]
...
So we've taken an arbitrary $k\dash$algebra and replaced it with a [simplicial%20k-algebra](simplicial%20k-algebra) which is given by polynomial rings in each degree, typically in infinitely many variables, which has the same homology.
This is the analog of a [projective%20resolution](projective%20resolution).
...
given by formally adjoining [filtered%20colimits](filtered%20colimits).
...
So starting with a functor defined on finitely generated polynomial rings, i.e. affine spaces, we get a simplicial [derived%20functors](derived%20functors).
For $R\in \mathrm{CAlg}_k$, using $S^\wait \mapsvia{\sim} R$, we can apply $F$ level-wise to get a new [simplicial%20object.md) $F(S^\wait](simplicial%20object.md)%20$F(S^/wait) \in \mathcal{C}$.
Then $LF(R)$ is defined by taking the colimit over $\Delta$ yields the [geometric%20realization.md](geometric%20realization.md), i.e. 
\[
LF(R) \da \hocolim_{\Delta} F(S^\wait) = \abs{F(S^\wait)}
\]
So we can promote functors on polynomial rings to functors on [simplicial%20commutative%20rings](simplicial%20commutative%20rings).
This ends up being a [Kan%20extension.md](Kan%20extension.md).
...
## The [cotangent%20complex.md](cotangent%20complex.md)
...
& 
\end{tikzcd}
Take [Kahler%20differentials](Kahler%20differentials) degree-wise to get 
$\Omega^1_{S^\wait/k}$.
...
We can now use [cotangent%20complex.md](cotangent%20complex.md) of $R/k$** as
\[  
...
There is a natural [Enriched%20category.md](Enriched%20category.md) in topological spaces, so we'll write $\Map$ for homs with their topological structure.
...
where the latter is in the category of simplicial commutative $k\dash$algebras with a *fixed* map to $S$, a bit like a [comma%20category](comma%20category).
This makes sense because $R\to S$ is a fixed map, and $S\oplus M$ has a projection to $S$, i.e. this is a square zero extension.

:::{.remark}
The point of [simplicial%20commutative%20rings](simplicial%20commutative%20rings) is that this extension still makes sense, even when $M$ is a chain complex.
It's still possible to make this into a simplicial commutative ring.
This may look familiar from the exercises in Hartshorne, since it resembles the definition of [Kahler%20differentials](Kahler%20differentials).
:::
...
So the RHS [comma%20category) mapping space in the previous equivalence is denoted the "space of derivations of $R$ and $M$" (whatever that means](comma%20category)%20mapping%20space%20in%20the%20previous%20equivalence%20is%20denoted%20the%20"space%20of%20derivations%20of%20$R$%20and%20$M$"%20(whatever%20that%20means)
:::


:::{.exercise title="?"}
You can base-change the [cofiber%20sequence.md](cofiber%20sequence.md)
...
,\]
which takes a polynomial ring $R$ to its algebraic [de%20Rham%20complex](de%20Rham%20complex).
...
Then $\dR_{R/k} \cong k$ by the [Poincaré%20lemma](Poincaré%20lemma), just the cohomology of affine space.
Then $L\dR_{S/k} \cong k$, which is disappointing.
:::

### The Hodge Filtration

How to fix this: we have the [Hodge%20filtration](Hodge%20filtration) $F_H^\wait \dR_{R/k}$, and the graded pieces are given by 
\[
\gr^i_H \dR_{R/k} \cong \Omega^i_{R/k}[-i] \cong \Lambda^i \Omega^1_{R/k}
,\]
a shifted version of the $i$th exterior power 1-forms.
Taking these gradings is compatible with colimits, so we can remember the filtration in the [Kan%20extension.md](Kan%20extension.md), yielding a canonical Hodge filtration on $L\dR$, $F^*_H L \dR_{\wait/k}$.
...
This follows because doing the [Kan%20extension.md](Kan%20extension.md) on the graded pieces is deriving the functor $\gr^i_H \dR_{\wait/k}$.
...
Note that this definition is a general way of taking [completion.md](completion.md).[^what_its_like]

[^what_its_like]: 
This is like defining the [p-adic%20integers.md](p-adic%20integers.md) as the limit of $\ZZ/p^n\ZZ$.


:::{.theorem title="Bhatt, Grothendieck, Hartshorne"}
Suppose $X/\CC$ is [finite%20type.md](finite%20type.md), then
\[
...
A positive feature is that this doesn't depend on a choice of putting $X$ in an ambient smooth [scheme.md](scheme.md).
:::
