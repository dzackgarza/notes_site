...
- Define $\Ld \Omega^1 _{A/R}$ to be the [[Unsorted/cotangent complex]] and take derived exterior powers for the other degrees.
...
- Fact: de Rham complex has a universal property, initial (strictly, so odd elements square to zero) commutative [[../differential graded algebras]] receiving a map $A\to X^0$, so the initial way to turn an algebra into a DGA.
...
![image_2021-05-06-11-25-44](_attachments/image_2021-05-06-11-25-44.png)
  ...
  - Why does this happen? $\HH(A/R)$ is the initial [[../simplicial algebra]] with an $S^1\dash$action receiving a map from $A$.
  - Analogies
...
- Produce a virtual fundamental class for flow categories.
  Moduli spaces of trajectories appearing will be "derived [[../orbifold]]".
...
- See the [[../ambidexterity]].
  - Orientability hypothesis allows using the [[equivariant cobordism]].
...
- Technical issues with loop actions on categories, only get module structure on homotopy category?
  - Also issues with local-to-global coherence, need $\hocolim$ to be compatible with Kuranishi atlases?
    And technical tools like [[norm map]].
...
- Can get the [[../Arnold conjecture]] from this homotopy type!
  ...
  - Recover statement about $\HF_p$ cohomology using the [[../Atiyah Hirzebruch spectral sequence]].
...
> Reference: Raphaël Beuzart-Plessis, On the spectral decomposition of the Jacquet-Rallis trace formula and the [Gan-Gross-Prasad conjecture](../Gan-Gross-Prasad conjecture.md) for unitary groups.
  BC NT/AG Seminar.
...
- Theorem: from [[../Langlands]] philosophy.
  There is a [[L packet]] where $P$ is nonvanishing.
